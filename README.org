...
This repository contains the folder count min that generates a mutidimensional array #define 4 by 131072 where the packet flows
are hashed and saved.
To hash the packet flow (ip_src, ip_dst, proto, src_port, dst_port) i used xxHash.
